...
export default function TableOrder() {
  ...
  const fetchData = async () => {
    ...
    const response = await authApi(token).get(endpoints["orders"]);
    if (response) {
  ...
  const handleSearch = async (e) => {
    ...
    const response = await authApi(token).get(
      endpoints["search-order"](keyword),
    );
    setData(response.data.orders);
  };
  ...
  const orderBy = async (type) => {
    ...
    const token = Cookies.get("token");
    const response = await authApi(token).get(
      endpoints["order-orders"](type, direction),
    );
    setData(response.data.orders);
  };
  ...
  const deleteItem = async (id) => {
    const token = Cookies.get("token");
    const response = await authApi(token).delete(endpoints["order-detail"](id));
    if (response.status === 204) {
      toast.success(response.data.message || "Deleted", {
        onClick: () => {
          setOrderContext((prev) => ({ ...prev, updateItem: true }));
        },
        onClose: () => {
          setOrderContext((prev) => ({ ...prev, updateItem: true }));
        },
  ...
  const handleWhenAddOrUpdate = () => {
    if (orderContext.updateItem === true) {
      fetchData();
      orderContext.updateItem = false;
    }
    if (orderContext.addItem === true) {
      fetchData();
      orderContext.addItem = false;
    }
  };
                          ...
                          >
                            ...
                            <td className="relative p-2">
                              <div
                                ...
                                className={`flex items-center gap-1 rounded-md ${d.status === "paid" ? "bg-green-500" : "bg-yellow-500"} px-1 py-2 text-xs text-white`}
                              >
                                <svg
